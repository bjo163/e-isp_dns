...
import { Shield, RefreshCw, AlertTriangle, KeyRound } from "lucide-react";
import { login, setToken, getToken, changePassword } from "@/lib/api-client";
...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.must_change_password) {
        // Store token temporarily for the password change API call
        setToken(res.token);
        setMustChange(true);
      } else {
        setToken(res.token);
  ...
  async function handleChangePw(e: React.FormEvent) {
    ...
    if (newPw === password) { setError("Password baru harus berbeda dari password lama"); return; }
    setChangingPw(true);
    try {
      ...
      <div className="w-full max-w-sm px-4">
        ...
        {mustChange ? (
          <form
            ...
            style={{ borderColor: "var(--brand-accent)", background: "var(--brand-card-bg)" }}
          >
            <div className="flex items-center gap-2 text-xs px-3 py-2.5 rounded border"
              style={{ borderColor: "rgba(99,102,241,0.4)", background: "rgba(99,102,241,0.06)", color: "var(--brand-accent)" }}>
              <KeyRound className="w-3.5 h-3.5 shrink-0" />
              Anda menggunakan password default. Silakan ganti password sebelum melanjutkan.
            </div>
            ...
            <button type="submit" disabled={changingPw}
              ...
              {changingPw ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <KeyRound className="w-3.5 h-3.5" />}
              {changingPw ? "Menyimpan..." : "Ganti Password & Masuk"}
            </button>
          </form>
        ) : (
          /* Login Form */
